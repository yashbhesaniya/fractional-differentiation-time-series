...
class Indicators(BaseEstimator, TransformerMixin):
    ...
    def __cal_SharpRatio (self, 
                        col_name: str ) -> None:
        """
        (WIP) --- Not yet in use.---
        Calculates the Sharpe Ration of the asset and creates new columns named 
            "SharpeRatio" in the data-frame passed as argument. For compatibility 
            purposes, it was added the ticker label in front of all columns 
            created.
        ...
        Args:
            ...
            data (pd.DataFrame): Columns of dataframe containing the variable 
                to be processed.
            col_name (str): name of the column with returns of the asset.
        Returns:
            None.

        """
        values = self.__data[col_name+"_returns"].values
        df_wrk = pd.DataFrame(values)
        df_wrk.columns = [col_name+"_returns"]
#        data[col_name+"_SharpeRatio"] = df_wrk[col_name].rolling(self.__sr_days, min_periods=1).mean()
#        data[self.__ticker+"_V_MA_10"] = df_wrk["volume"].rolling(10, min_periods=1).mean()
#        data[self.__ticker+"_V_MA_20"] = df_wrk["volume"].rolling(20, min_periods=1).mean()
